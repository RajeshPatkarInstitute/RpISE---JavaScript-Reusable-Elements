// var stats = require('./Own_Libraries/Statistics/Statistics');
var nj = require('./Own_Libraries/NumpyJS/NumpyJS');
var mlOperations = require('./Own_Libraries/MLOperations/MLOperations');
...
var MultipleLinearRegression =
  function() {
    function predict(to_predict) {
      var ans = [];
      for (k in to_predict) {
        ans[k] = (this.beta[0] + nj.sum(nj.ElementMultiply(this.beta.slice(1), to_predict[k])));
      }
      return ans;
    }
    ...
    function gradientDescent(verbose = false) {
      ...
      while (this.isConverage() == false) {
        ...
        for (var j = 0; j < this.beta.length; j++) {
          ...
          var t = nj.ElementMultiply(x_temp, temp);
    ...
    function calculateYHat() {
      for (var i in this.y) {
        ...
        var outer_temp = nj.ElementMultiply(this.beta, this.x[i]);
